...
u8 PS2_Keyscan(u8 mode)
{
	static u8 ps_key_up=1,key;//按键按松开标志
	if(mode)ps_key_up=1;
  //支持连按		  
	key=PS2_DataKey();
	if(ps_key_up&&key!=0)
	...
	}else if(key==0)ps_key_up=1; 	    
 	return 0;// 无按键按下
}

u8 PS2_Keyscan2(u8 mode)
{
	static u8 ps_key_up2=1,key;//按键按松开标志
	if(mode)ps_key_up2=1;
  //支持连按		  
	key=PS2_DataKey();
	if(ps_key_up2&&key!=0)
	{
