...
u8 MPU_Get_Gyroscope(short *gx,short *gy,short *gz);//获取原始陀螺仪数据
u8 MPU_Get_Accelerometer(short *ax,short *ay,short *az);//获取原始加速度数据
u8 DMP_Init(void);                                   //初始化DMP
u8 Read_DMP(float *pitch,float *roll,float *yaw);                     //获取DMP数据
void Read_DMP1(void);
float Read_Kalman(u8 who);                     //卡尔曼滤波获取数据  0-pitch 1-roll 2-yaw
#endif
